#!/bin/bash
set -ex

FQDN=$(hostname)

ls /etc/yezzey-ssl/truststore.jks && exit 0 || true
mkdir /etc/yezzey-ssl

echo "-----BEGIN CERTIFICATE-----

nAWt08+eToI1YUvjl6UT0bpVaJEACv+/HfBr1T26u4Jh
-----END CERTIFICATE-----" > /etc/yezzey-ssl/ca.cert.pem

echo "-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----" > /etc/yezzey-ssl/ca.key

openssl genrsa -out /etc/yezzey-ssl/private.key -passout pass:testpassword123 4096
openssl req -new -key /etc/yezzey-ssl/private.key -out /etc/yezzey-ssl/server.csr -passin pass:testpassword123 -subj "/C=RU/ST=Test/L=Test/O=Test/OU=Test/CN=${FQDN}"
openssl x509 -req -days 365 -in /etc/yezzey-ssl/server.csr -CA /etc/yezzey-ssl/ca.cert.pem -CAkey /etc/yezzey-ssl/ca.key -CAcreateserial -out /etc/yezzey-ssl/public.crt -passin pass:testpassword123

chmod 755 /etc/yezzey-ssl/*