...
bool S3InterfaceService::parseBucketXML(ListBucketResult *result, xmlParserCtxtPtr xmlcontext,
    ...
    if (is_truncated && next_marker) marker = next_marker;
    else marker = (is_truncated && key) ? key : "";
...
string S3InterfaceService::uploadPartOfData(S3VectorUInt8 &data, const S3Url &s3Url,
    ...
    if (resp.getStatus() == RESPONSE_OK) {
        ...
        auto res = std::search(headers.begin(), headers.end(), toSearch.begin(), toSearch.end(), [](char ch1, char ch2){
             return std::tolower(ch1) == std::tolower(ch2);
        });

        if (res == headers.end()) {
            S3_DIE(S3RuntimeError, "responce does not contain etag in header");
        }
